   ...
   <div>
    ...
    <v-container>
        <v-row dense>
         <v-col md="3">
                <div>
                    <v-text-field v-model="search" prepend-inner-icon="mdi-magnify" outlined clearable placeholder="search"></v-text-field>
                    <v-list subheader color="transparent" v-if="$vuetify.breakpoint.mdAndUp">
                     <v-subheader>Categories</v-subheader>
                     <v-list-item v-for="(c, i) in categories" :key="i" link>
                            <v-list-item-avatar>
                ...
                <template v-for="(p, i) in filteredProducts" >
                    <v-col cols="12" md="6" :key="`products${p.id}-${i}`">
                        <v-card nuxt :to="`/products/${p.id}`"  link color="surface" class="el ma-2 mb-5 mr-5 ">
                            ...
                            <v-card-text>
                                <v-chip x-small label outlined class="mr-1"  v-for="(t,i) in p.tags" :key="i">
                    {{ t }}
                 </v-chip>
                            </v-card-text>
                        </v-card>
